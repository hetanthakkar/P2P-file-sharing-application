...
/**
 * This class implements a 2D matrix of numbers using a 2D array. This implementation is efficient if most of the
 * entries of the matrix are not zero. But this wastes a lot of space and computing time if most of its entries are zero
 */
public class ArrayMatrix implements SquareMatrix {
    ...
    public void set(int i, int j, float value) throws IllegalArgumentException {
        if ((i<0) || (i>=mat.length)) {
            throw new IllegalArgumentException("Row number in set cannot be beyond the bounds of the matrix");
        }

        if ((j<0) || (j>=mat[i].length)) {
            throw new IllegalArgumentException("Column number in set cannot be beyond the bounds of the matrix");
        }
        mat[i][j] = value;
    }
    ...
    public float get(int i, int j) throws IllegalArgumentException {
        if ((i<0) || (i>=mat.length)) {
            throw new IllegalArgumentException("Row number in get cannot be beyond the bounds of the matrix");
        }

        if ((j<0) || (j>=mat[i].length)) {
            throw new IllegalArgumentException("Column number in get cannot be beyond the bounds of the matrix");
        }
        return mat[i][j];
    }
    ...
    public SquareMatrix postmul(SquareMatrix other) throws IllegalArgumentException {
        if (this.size()!=other.size()) {
            throw new IllegalArgumentException("The size of this matrix is not the same as the size of the other matrix," +
